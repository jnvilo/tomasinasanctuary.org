...
<?php
    if ($artcats->is_subcategories_on() && !$artcats->is_articles_mode()) {
        $document =& JFactory::getDocument();
        $url = JURI::base().'modules/mod_artcats/tmpl/css/menustyle/'.$artcats->getMenustyleTemplate();
...
<?php if ($artcats->is_combine_on() && !$artcats->is_articles_mode()) : ?>
    ...
    <?php elseif ($artcats->is_subcategories_on()) : ?>
        <?php $artcats->getDisplayableMenustyle($list); ?>
    <?php endif; ?>
