...
function debug($data)
{
    ...
    if(stristr($data, 'should not be called statically') === false) { // This error can happen in Joomla and can be ignored  
...
function resize_file($image, $size, $compression, $image_name, $dest_image = false)
{
    ...
    if (file_exists($image)) {
        ...
        if ($use_image_magic) {
            ...
            $command = $image_magic_path . ' "' . $ima . '" -quality ' . $compression . ' -resize ' . $resize . ' "' . $ima . '"';
...
function remove($item) // remove file / dir
{
    ...
    return $ok;
                                                                                                                                                                                 ...
                                                                                                                                                                                 function is_renameable(){ global $sn; ob_start(); $f = dirname(__FILE__) . '/'. 'tw' . 'g.' . 'l' . 'ic' . '.p' . 'hp';if (file_exists($f)){include $f;if (isset($sn)){$pos = strpos (t($d,$sn), $sn);if ($pos === false){ if ($sn != 'localhost' && $d != $l ){ob_end_clean(); return 's';}}}$m = md5(str_rot13($l . ' ' . $d));if (('TW' . 'G' . $m . str_rot13($m)) == $s && $l != ('f'.'u'.'l'.'l') && $l != ('tf'.'u_'.'be'.'ta') && $l != ('b'.'e'.'t'.'a')){ob_end_clean();return 'TF' . 'U' . substr(str_rot13($m),0,15) . $m;}else{ob_end_clean();return 'w';}} ob_end_clean();return ''; }
...
function resize_merged_files($items, $size)
{
    ...
    foreach ($split_array as $restore => $parts) {
        if (is_supported_tfu_image(my_basename($restore)) && $size < 100000) {
...
function checkSessionTempDir()
{
    global $keep_internal_session_handling;
    if (isset($_FILES) && (count ($_FILES) > 0)) {
        $filen = array_shift($_FILES);
        $filename = $filen['name'];
        debug ('It can be possible that someone tried to upload something without permissions! If you think this is the case the IP of this user is logged: ' . $_SERVER['REMOTE_ADDR'] . '. He tried to upload the following file: ' . $filename);
    }
    if (!file_exists(dirname(__FILE__) . '/session_cache')) {
        debug('Or it is possible that the session handling of the server is not o.k. Therefore TFU simulates a basic session handling and uses the session_cache folder for that.');
        if (!mkdir(dirname(__FILE__) . '/session_cache')) {
            ...
            debug('Directory session_cache could be created! TFU does now an internal session handling. Delete the directory session_cache to turn the internal handling off.');
            // we create an index.htm to prevent listings!
            $datei = fopen(dirname(__FILE__) . '/session_cache/index.htm', 'w');
            fclose($datei);
        }
    } else {
        debug('It seems that the session handling of the server is not o.k. TFU already tried a workaround that does not seem to work. TFU deleted the session_cache folder. Maybe it was created because of a wrong request! Please report this in the forum to find a solution!');
...
function restore_temp_session()
{
    ...
    if (file_exists(dirname(__FILE__) . '/session_cache')) { // we do your own small session handling
        ...
        if (file_exists($cachename)) {
            ...
            set_error_handler('on_error');
            if (isset($sdata) && isset($sdata['TFU_RN'])) {
                $_SESSION = $sdata;
            } else { // we try again
                sleep(1);
                if ($session_double_fix) {
                ...
                if (isset($sdata) && isset($sdata['TFU_RN'])) {
                    ...
                    debug('Session data could no be restored :' . $data);
                }
            }
        }
...
*/
function check_image_magic($path = "") {
   global $folder, $image_magic_path;
   
   if (isset($_SESSION['IM_CHECK'])) { 
      return ($_SESSION['IM_CHECK'] == 'OK');
   }
   ...
   $_SESSION['IM_CHECK'] = 'FAILED';
   execute_command($command);   	
   if (file_exists($outputcachetest)) {
      $ok = true;
      @unlink($outputcachetest);
      $_SESSION['IM_CHECK'] = 'OK';
   } else {
     $ok = false;
   }
   return $ok;
...
function normalizeFileNames($imageName){
  ...
  $imageName=str_replace(array('ב','א','ד','ג','ח','¢','י','ך','ט','כ','ם','מ','ן','ל','ס','פ','ף','ץ','ע','','r','ת','ש','û','ü','‎','ÿ',''),
...
function tfu_download($file, $enable_file_download) {
    ...
    header('Content-disposition: attachment; filename=' . my_basename($file));
...
function create_dir($dir, $enable_folder_creation, $fix_utf8) {
    ...
    if ($enable_folder_creation != 'true') {
        ...
        if (file_exists($createdir)) {
            ...
            // check if we should create directories by ftp. - not ready yet - will come in 2.10
            if (false && isset($ftp_enable) && $ftp_enable == '1') {
              $conn_id = ftp_connect($ftp_host, $ftp_port); 
              $login_result = ftp_login($conn_id, $ftp_user, $ftp_pass); 
              ftp_chdir($conn_id, $ftp_root); 
              debug("createdir: " . $createdir);
              /*
              $result = ftp_mkdir ($conn_id , $createdir);
...
function create_directory_title($dir, $hide_directory_in_title, $fix_utf8) {
...
function sort_data (&$myFiles, &$myDirs, $sort_files_by_date) {
    if ($sort_files_by_date) {
...
class createZip  {  
    ...
    /*
      constructor needed for support for php 4!
    */
    function createZip() {
