<?php
/**
* @version		$Id: controller.php 10906 2008-09-05 07:27:34Z rhuk $
* @package		RokCandy
...
require_once (JPATH_COMPONENT_ADMINISTRATOR.DS.'helpers'.DS.'rokcandyhelper.php' );

class RokCandyModelRokCandy extends JModel
{
	/**
	 * Category ata array
	 *
	...
	function _buildContentWhere()
	{
		...
		if ($search) {
			$where[] = 'LOWER(a.macro) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
			$where[] = 'LOWER(a.html) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
		}
		if ( $filter_state && JRequest::getVar('task') != 'list') {
			if ( $filter_state == 'P' ) {
				$where[] = 'a.published = 1';
